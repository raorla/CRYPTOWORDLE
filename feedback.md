...
- **`@iexec-nox/handle` auto-configures supported networks.** On Ethereum Sepolia, `createViemHandleClient(walletClient)` just works: gateway, subgraph and NoxCompute address all resolve from the chainId (see friction item 3 for the docs-side catch).
...
**Fix:** encrypted bitwise ops (`and`/`or`/`xor` at minimum), and an encrypted-randomness primitive (a TEE-generated random euint with an attested seed would fit the existing trust model).
...
The plugin is Hardhat 3-only (HH3 plugin API), ESM-only, Node 22+. Most existing Ethereum guides, tutorials and team codebases are still Hardhat 2 / CJS. HH3's changed config format (`defineConfig`, `plugins` array, network `type`/`chainType`) plus ESM test-runner quirks (e.g. we had to pin `@nomicfoundation/hardhat-node-test-runner` via `overrides`) make the first day steeper than it needs to be, before you've written a single line of confidential code.
...
4. The Ethereum Sepolia auto-config callout is stale (the SDK already ships it).
...
3. **Batch ACL grants**: `allowThis` and `allowPublicDecryption` over a `bytes32[]`. Our guess does 12 individual ACL calls (2 × 5 colors + 2 × win) that could be one.
...
*Bottom line: the core protocol delivered everything CryptoWordle's design needed (sealed state, on-ciphertext compute, trustlessly verifiable decryption). Close the version-drift and docs gaps, ship the two SDK helpers everyone is hand-writing, and this becomes a toolchain you can drop a hackathon team into on Friday night and get a confidential dApp by Sunday. We know, because that's roughly what happened, minus the hours listed above.*
